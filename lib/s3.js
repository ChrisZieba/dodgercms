        ...
        if (source.substr(-1) === '/') {
            ...
            listObjects(source, bucket, function(err, data) {
                if (err) {
                    ...
                    console.log(source, target, contents)
                    ...
                    async.each(contents, function(object, cb) {
                        ...
                        console.log(source, object.Key, targetPrefix);

                        // replace the source with the target
                        var key = targetPrefix + object.Key.substr(source.length);
                        console.log(key);

                        //console.log(object.Key.split(source))
                        // add the slash back that was removed
                        
                        

                        dodgercms.s3.copyObject(source, key, bucket, cb);
                    }, function(err) {
