...
dodgercms.auth = (function() {
  ...
  */
  var getCredentials = function(allOrNothing) {
    var credentials = {
      ...
      region: localStorage.getItem('dodgercms-region'),
    };
  ...
  */
  function login(params, callback) {
    ...
    var required = ['siteBucket', 'accessKey', 'accessSecret', 'region'];
    ...
    var dataBucket = params.dataBucket || 'data.' + params.siteBucket;
    var assetsBucket = params.assetsBucket || 'assets.' + params.siteBucket;

    // This is the same policy as the IAM user from where the token was generated
    var policy = getPolicy(dataBucket, assetsBucket, params.siteBucket);
    ...
    sts.getFederationToken(stsParams, function(err, data) {
      if (err) {
        ...
        async.each([dataBucket, assetsBucket], function(bucket, cb) {
          ...
          if (err) {
            ...
            function(err, results) {
              if (err) {
                ...
                sessionStorage.setItem('dodgercms-token-region', params.region);
                ...
                localStorage.setItem('dodgercms-region', params.region);
