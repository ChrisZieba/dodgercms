...
dodgercms.auth = (function() {
  ...
  */
   ...
   * @return {String}
  */
  var getEndpoint = function(bucket) {
    var endpoint = 'https://s3.amazonaws.com/' + bucket + '/';
  ...
  */
  function login(params, callback) {
    ...
    sts.getFederationToken(stsParams, function(err, data) {
      if (err) {
        console.log(err);
        return callback('Access Denied. Please make sure the acccess key and secret are correct and that permission for the action "sts:GetFederationToken" is allowed.');
      } else {
        var s3 = new AWS.S3({
          ...
          if (err) {
            ...
            function(err, results) {
              if (err) {
                ...
                var endpoint = getEndpoint(params.siteBucket);
